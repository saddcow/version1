...
import 'package:try1/src/api/api.dart';
import 'package:try1/src/features/weather/data/api_exception.dart';
import 'package:try1/src/features/weather/data/weather_repository.dart';
import 'package:try1/src/features/weather/domain/weather/weather.dart';
...
void main() {
  test('repository with mocked http client, success', () async {
    ...
    when(() => mockHttpClient.get(api.weather('Naga City'))).thenAnswer(
        (_) => Future.value(http.Response(encodedWeatherJsonResponse, 200)));
    final weather = await weatherRepository.getWeather(city: 'Naga City');
    // expectations
  ...
  test('repository with mocked http client, failure', () async {
    ...
    final weatherRepository =
        HttpWeatherRepository(api: api, client: mockHttpClient);
    when(() => mockHttpClient.get(api.weather('Naga City')))
        .thenAnswer((_) => Future.value(http.Response('{}', 401)));
    // expectations
    expect(() => weatherRepository.getWeather(city: 'Naga City'),
